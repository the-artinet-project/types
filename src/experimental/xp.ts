 ...
 */
export const RoleSchema = z.enum(['user', 'agent', 'developer', 'system', 'assistant', 'tool', 'function']);
/**
 * @deprecated This is an experimental type, subject to rapid modification and its location will change.
 */
 ...
 * @deprecated This is an experimental type, subject to rapid modification and its location will change.
 */
export const FunctionSchema = BaseSchema.required()
    .extend({
...
export const isFunctionCall = (call: unknown): call is FunctionCall => FunctionCallSchema.safeParse(call).success;
...
export const isCustomToolCall = (call: unknown): call is CustomToolCall => CustomToolCallSchema.safeParse(call).success;

/**
 * @deprecated This is an experimental type, subject to rapid modification and its location will change.
 */
//TODO: Add support for audio responses
export const MessageSchema = Session.MessageSchema.extend({
    role: z.literal(Session.MessageRole.assistant),
    content: z.string().nullable(),
    tool_calls: z
        .array(z.discriminatedUnion('type', [FunctionCallSchema, CustomToolCallSchema]))
        .optional()
        .nullable(),
});
...
export const isMessage = (message: unknown): message is Message => MessageSchema.safeParse(message).success;
