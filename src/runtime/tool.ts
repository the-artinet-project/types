...
export const ToolServerSchema = ServiceConfigSchema.extend({
    ...
    //TODO: Next Major Version, info will be optional
...
/**
 * @deprecated This will be removed in the next major version. Use ToolServerSchema instead.
 */
export const ToolServerInputSchema = ToolServerSchema.partial({
    info: true,
});
/**
 * @deprecated This will be removed in the next major version. Use ToolServer instead.
 */
export type ToolServerInput = z.output<typeof ToolServerInputSchema>;

export const MCPStdioArgumentsSchema = z.object({
    command: z.string().describe('The command to be used to start the tool.'),
    args: z.array(z.string()).describe('The arguments to be used to start the tool.'),
});
...
export type ToolInstanceArguments = z.output<typeof ToolInstanceArgumentsSchema>;
...
export const ToolInstanceSchema = ToolServerSchema.partial({
    ...
    arguments: ToolInstanceArgumentsSchema.optional().describe('The arguments to be used to start the tool.'),
...
export const isToolServer = (tool: unknown): tool is ToolServer => ToolServerSchema.safeParse(tool).success;

/**
 * @deprecated This will be removed in the next major version. Use ToolInstance instead.
 */
export const isToolServerInput = (tool: unknown): tool is ToolServerInput =>
    ToolServerInputSchema.safeParse(tool).success;

export const isToolInstance = (tool: unknown): tool is ToolInstance => ToolInstanceSchema.safeParse(tool).success;

export const isToolService = (service: unknown): service is ToolService => ToolServiceSchema.safeParse(service).success;
...
export const ToolCallSchema = ToolInstanceSchema.required({
    ...
    .extend({
        call: MCP.CallToolRequestParamsSchema.describe('The call to invoke the tool.'),
        content: MCP.ToolUseContentSchema.optional().describe('The content of the tool call.'),
    });
...
export const isToolRequest = (req: unknown): req is ToolRequest => ToolRequestSchema.safeParse(req).success;
...
export const ToolCallResultSchema = ToolCallSchema.partial({
    ...
    content: MCP.ToolResultContentSchema.optional().describe('The content of the tool result.'),
...
export const ToolResponseSchema = ToolCallResultSchema.extend(WithKindSchema.shape)
    .extend(CallerIdSchema.shape)
    .extend({
...
export const isToolResponse = (res: unknown): res is ToolResponse => ToolResponseSchema.safeParse(res).success;
